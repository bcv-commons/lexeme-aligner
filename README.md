...
## The main dataset: `publish/lexeme-alignments`
...
[**publish/lexeme-alignments/README.md**](publish/lexeme-alignments/README.md) — read that first if you're building
...
`publish/lexeme-alignments` and most companions are **CC0-1.0** — derived counts and statistics, not the
running text of any translation, so no copyrightable expression is reproduced. Each dataset's own
README has the exact terms; `publish/lexeme-alignments/manifest.json` links each language's source-text
