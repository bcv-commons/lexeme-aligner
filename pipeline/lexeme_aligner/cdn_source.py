...
Two stages:
  ...
  2. **decode → USJ** (`--to-usj`, the one Node edge) — PKF→USFM via a Proskomma converter (Node),
    ...
    python3 -m lexeme_aligner.cdn_source --iso ind --to-usj pipeline/work/ingest-cache/usj-ind \
...
def main() -> int:
    ...
    ap.add_argument("--pool", type=Path, default=Path("pipeline/work/ingest-cache/pkf-pool"), help="PKF download cache (git-ignored)")
    ...
    write_pin(pin, args.pin or Path("config/pins") / f"{args.iso}.json")
    if args.sources:
