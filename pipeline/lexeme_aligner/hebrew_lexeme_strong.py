...
_TABLE_PATH = Path("config/hebrew_lexeme_strong.json")
_MIN_N = 10
_MIN_PURITY = 0.90
...
def build_table(out_dir: Path = Path("pipeline/work/out"), gold_path: Path = Path("pipeline/vendor/resources/strongs/attestations/eng.parquet"),
                iso: str = "eng", tag: str = "merged", min_n: int = _MIN_N, min_purity: float = _MIN_PURITY,
                spine_db: Path = Path("pipeline/lexeme-spine.db")) -> dict:
