...
_EXCLUSIONS = Path("config/language_exclusions.json")
_EDITIONS_CONFIG = Path("config/language_editions.json")
...
def _tag(iso: str, edition_code: str, is_primary: bool) -> str:
    """Stable per-edition tag, derived from the edition's OWN edition_code (a helloAO translation id
    or DBT bible_id — a stable, globally-unique identifier), NOT its position in the list, NOT whether
    it's currently the only edition, and NOT (as of 2026-07-25) whether it's the primary one. This
    matters: adding/removing/reordering editions in `data/language_editions.json` never reassigns
    another edition's tag to a DIFFERENT edition's cached data — only the changed edition's own
    ingest+align needs to (re)run; the final `export_lex --pool` step just re-aggregates whichever
    tags are currently in play. An index-based scheme (spa_1, spa_2, ...) would NOT have this
    property — inserting/removing an entry mid-list would shift every later index onto someone else's
    cache. Bare-iso primary tags (the pre-2026-07-25 scheme) were retired project-wide on that date —
    all 199 languages published under it were renamed to edition_code-derived tags (data/
    legacy_bare_iso_tags.json records what each one's edition_code was, for reference), so this
    function no longer needs an exception at all."""
...
def _pin_path(tag: str) -> Path:
    return Path("config/pins") / f"{tag}.json"
...
def main() -> int:
    ...
    # already_exported(iso) in onboard_batch.py/onboard_catalog.py can never find on a resumed run —
    # every already-done language would look "not done" and get needlessly re-onboarded. A brand new
    # language has no prior published identity to preserve, so the bare iso IS the right default here
    # (unlike a rename-tolerant existing publish, where --publish-iso is set explicitly per call).
    export_args: list[object] = ["--iso", primary, "--publish-iso", args.iso, "--method", args.method]
    if pool:
