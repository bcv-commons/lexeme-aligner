...
The `publish/lexeme-alignments` producer (`lexeme_aligner.export_lex`) aggregates the per-verse `.jsonl` into the
...
2. ~~`publish/lexeme-alignments` producer (`surface, lexeme, strong, method, count, share, hi_conf`).~~ **Done** —
...
8. ~~Lexeme anchoring (on the MACULA spine).~~ **Landed + working.** shoresh shipped
   `pipeline/lexeme-spine.db` (MACULA, 607k words; `lexeme = lang:augmented-strong`, `strong` = rollup,
...
higher-confidence `publish/lexeme-alignments`. Each verse is a training pair `source = Strong's sequence`
...
- **(b) Align each version separately, then merge the `publish/lexeme-alignments`** — union the `(surface→Strong's)`
...
per-testament, merge into one enriched `publish/lexeme-alignments/iso=<iso>/` with a cross-version agreement signal.
